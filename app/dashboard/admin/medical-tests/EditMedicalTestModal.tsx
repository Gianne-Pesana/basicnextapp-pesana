...
interface EditMedicalTestModalProps {
  isOpen: boolean;
  onClose: () => void;
  onEdit: (data: Omit<MedicalTest, 'uom_name' | 'category_name'>) => Promise<void>;
  test: MedicalTest | null;
  uoms: Uom[];
  categories: TestCategory[];
}

export default function EditMedicalTestModal({ isOpen, onClose, onEdit, test, uoms, categories }: EditMedicalTestModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!test || !formData.name.trim()) return;
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
      ...
      >
        <div className="bg-amber-600 px-4 py-3 border-b flex items-center justify-between cursor-move select-none" onMouseDown={onMouseDown}>
          <div className="flex items-center gap-2 text-white font-semibold">
            <GripHorizontal size={20} className="text-white/70" />
            <span>Edit Medical Test</span>
          </div>
          <button onClick={onClose} className="text-white/70 hover:text-white transition-colors p-1 rounded hover:bg-amber-700">
            <X size={20} />
          </button>
        </div>
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          <div className="grid grid-cols-2 gap-4">
            ...
            <div className="col-span-2 space-y-1">
              ...
              <textarea
                className="w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-amber-500 outline-none h-20"
          ...
          <div className="flex justify-end gap-3 pt-4 border-t">
            <button type="submit" disabled={isSubmitting || !formData.name.trim()} className="px-6 py-2 text-sm font-medium text-white bg-amber-600 rounded-lg hover:bg-amber-700 disabled:opacity-50">
